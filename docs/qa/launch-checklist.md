...
| 0.7 | **Role-gated permissions** | Staff cannot access Settings, cannot reassign others' tasks, cannot approve own requests. Manager scoped to department. Admin full access within practice. | Playwright — settings access [`tests/e2e/permissions/settings-access.spec.ts`](../../tests/e2e/permissions/settings-access.spec.ts); task visibility scope (admin/manager/staff) [`tests/e2e/permissions/task-scope.spec.ts`](../../tests/e2e/permissions/task-scope.spec.ts); task reassignment guard [`tests/e2e/permissions/task-reassign.spec.ts`](../../tests/e2e/permissions/task-reassign.spec.ts). Sub-case still open: request **approval** (staff cannot approve own request) — needs feature decision on what "approve" means in the requests flow before a spec can be written. |
