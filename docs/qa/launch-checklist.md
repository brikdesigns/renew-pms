...
**Closing the gap.** Add Playwright coverage for: (a) clicking the board checkbox on an overdue row and asserting the visual state survives the refetch, (b) toggling "Show resolved" with a seeded completed-yesterday row and asserting it appears, (c) the rollback path's toast carrying the API's error string (not the generic "Failed to update status"). Plus extend the seed helpers to include rows with `due_date = yesterday` and rows whose recurring instance was generated before today. To be propagated into [Notion — Renew Workflows](https://www.notion.so/Workflows-34e97d34ed2880e7a0bfe6f74d41b876) under the Tasks service so it's visible team-wide, not just in this repo.
