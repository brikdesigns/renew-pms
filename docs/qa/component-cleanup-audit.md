...
## Findings — 18 open / 42 originally / 38 verified across 9 categories
...
| 9 | `task/bds-cleanup-edittemplate-addable` | Adopt existing BDS `<AddableEntryList>` (or extended variant) for the EditTemplateSheet checklist authoring tab. | 1 renew-pms file (+ optional BDS PR if `renderTrailing` slot is needed) | Pre-work uncovered during Cat 3 #3 conversation: the EditTemplateSheet "Edit Checklist" tab is the **authoring** counterpart to the live ViewTaskSheet checklist (different UX role: define vs. complete). It hand-rolls a numbered row with `+ Link to Inventory` / Edit / Remove actions; BDS `AddableEntryList` covers most of this shape but lacks a per-row trailing-slot. Decision: (a) move inventory linking into a per-item drawer/menu and use vanilla `AddableEntryList`, (b) extend BDS with a `renderTrailing(entry, idx)` slot, or (c) ship a sibling `AddableEntryListWithActions`. Pre-work: Storybook MCP for current `Addable*` API surface; design call on inline-action vs. drawer. |
...
**Sequencing rationale:** Batches 1–3 are pure cleanup with no design decisions and no BDS PR — they can run in parallel with feature work. Batches 4–6 require Storybook MCP checks first. Batches 7–9 are cross-repo BDS promotions / extensions and need design alignment before code (Batch 9's design call is whether inventory linking stays inline or moves to a drawer).
