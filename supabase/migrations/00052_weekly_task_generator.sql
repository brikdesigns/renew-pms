-- =============================================================================
-- 00052: Weekly task generator
-- =============================================================================
-- Adds generate_weekly_tasks(p_practice_id) and run_weekly_tasks() following
-- the same structure as generate_daily_tasks (migration 00045).
--
-- ADR-002: skip+spawn-fresh rollover semantic.
--   - Stale-skip: frequency='weekly' tasks with due_date < current Monday.
--   - Spawn: one task per active weekly template per practice, due current Monday.
--   - Idempotency: skip if due_date = current Monday already exists for template.
--
-- ADR-003: fixed UTC cron (0 5 * * 1), no per-practice timezone column.
--   - current_date at 05:00 UTC is always Monday local time for US practices.
--   - task_reset_cadence column (00051) is informational at this stage; the
--     generator owns weekly templates by frequency, not by reset cadence.
-- =============================================================================


create or replace function public.generate_weekly_tasks(p_practice_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  -- Step A: stale-skip incomplete weekly tasks from prior week boundaries.
  -- Mirrors the daily generator's approach: mark anything spawned by a weekly
  -- template that has a due_date before this Monday as skipped.
  update public.tasks
  set status     = 'skipped',
      updated_at = now()
  ...
  -- Step B: spawn this week's instance for every active weekly template
  ...
  loop
    ...
    -- Idempotency: skip if this week's instance already exists.
    select exists(
    ...
    if v_template.display_mode = 'expanded' then
      ...
      loop
        insert into public.tasks (
          ...
          'not_started', v_template.priority, v_template.frequency, v_week_start
        );
      end loop;
...
create or replace function public.run_weekly_tasks()
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  loop
    perform public.generate_weekly_tasks(v_practice.id);
  end loop;
end;
$$;
